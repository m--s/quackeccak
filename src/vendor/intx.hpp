...
struct result_with_carry {
	...
	constexpr explicit(false) operator std::tuple<T &, bool &>() noexcept {
		return {value, carry};
	}
};
...
struct div_result {
	...
	/// Conversion to tuple of references, to allow usage with std::tie().
	constexpr explicit(false) operator std::tuple<QuotT &, RemT &>() noexcept {
		return {quot, rem};
	}
};
...
/// Addition with carry.
template <unsigned N>
constexpr result_with_carry<uint<N>> addc(const uint<N> &x, const uint<N> &y, bool carry = false) noexcept {
	uint<N> s;
	bool k = carry;
...
template <unsigned N>
constexpr result_with_carry<uint<N>> subc(const uint<N> &x, const uint<N> &y, bool carry = false) noexcept {
	uint<N> z;
	bool k = carry;
	for (size_t i = 0; i < uint<N>::num_words; ++i) {
...
public:
	...
	constexpr explicit(false) uint(T x) noexcept
	    requires std::is_convertible_v<T, uint64_t> : words_ {static_cast<uint64_t>(x), 0} {
	}

#if INTX_HAS_BUILTIN_INT128
	constexpr explicit(false) uint(builtin_uint128 x) noexcept : words_ {uint64_t(x), uint64_t(x >> 64)} {
	}
	...
	constexpr explicit operator Int() const noexcept requires std::is_integral_v<Int> {
	...
	constexpr uint &operator+=(uint y) noexcept {
		return *this = *this + y;
	}
	...
	friend constexpr uint operator~(uint x) noexcept {
		return {~x[0], ~x[1]};
	}
	friend constexpr uint operator|(uint x, uint y) noexcept {
	...
	constexpr uint &operator<<=(uint shift) noexcept {
...
constexpr div_result<uint64_t, uint128> udivrem_3by2(uint64_t u2, uint64_t u1, uint64_t u0, uint128 d,
                                                     uint64_t v) noexcept {
	auto q = umul(v, u2);
	q = fast_add(q, {u1, u2});
...
public:
	...
	template <unsigned M>
	constexpr explicit(false) uint(const uint<M> &x) noexcept requires(M < N) {
	...
	constexpr explicit(false) uint(T... v) noexcept
	    requires std::conjunction_v<std::is_convertible<T, uint64_t>...> : words_ {static_cast<uint64_t>(v)...} {
	}
	...
	template <unsigned M>
	constexpr explicit operator uint<M>() const noexcept requires(M < N) {
		uint<M> r;
		for (size_t i = 0; i < uint<M>::num_words; ++i)
			r[i] = words_[i];
		return r;
...
template <unsigned M, unsigned N>
[[gnu::always_inline]] constexpr normalized_div_args<M, N> normalize(const uint<M> &numerator,
                                                                     const uint<N> &denominator) noexcept {
...
/// r = x - multiplier * y.
constexpr uint64_t submul(uint64_t r[], const uint64_t x[], const uint64_t y[], int len, uint64_t multiplier) noexcept {
	// OPT: Add MinLen template parameter and unroll first loop iterations.
	INTX_REQUIRE(len >= 1);
...
constexpr void udivrem_knuth(uint64_t q[], uint64_t u[], int ulen, const uint64_t d[], int dlen) noexcept {
	INTX_REQUIRE(dlen >= 3);
...
constexpr div_result<uint<M>, uint<N>> udivrem(const uint<M> &u, const uint<N> &v) noexcept {
	...
	if (na.num_divisor_words == 1) {
		const auto r = internal::udivrem_by1(as_words(na.numerator), na.num_numerator_words, as_words(na.divisor)[0]);
		return {static_cast<uint<M>>(na.numerator), r >> na.shift};
	}

	if (na.num_divisor_words == 2) {
		const auto d = as_words(na.divisor);
		const auto r = internal::udivrem_by2(as_words(na.numerator), na.num_numerator_words, {d[0], d[1]});
		return {static_cast<uint<M>>(na.numerator), r >> na.shift};
	}
	...
	uint<M> q;
	internal::udivrem_knuth(as_words(q), &un[0], na.num_numerator_words, as_words(na.divisor), na.num_divisor_words);
...
inline T load(const uint8_t (&src)[M]) noexcept {
	static_assert(M == sizeof(T), "the size of source bytes must match the size of the destination uint");
	T x;
...
template <typename T, unsigned M>
inline T load(const uint8_t (&src)[M]) noexcept {
	static_assert(M <= sizeof(T), "the size of source bytes must not exceed the size of the destination uint");
	T x {};
